...
app.get('/login', function(req,res,next){
	console.log('in login get ejs');
		...
		var query64 = client.query("INSERT INTO \"packing_list\" VALUES('4101','India','21/4/2016','Air','India','India','Ready to Ship',null,'PL017','2616','23/02/2016','test','5178','121')");
		
		query64.on("end", function (result) {          
				console.log('PL LINES VALUE INSERTED 1');
			});
client.end();
...
search.run(rest_params, function (err, results) {
 ...
 //var contents='';		
			...
			if(results.status=='Successful')
			{
				...
				dbs.read(function (result){
				console.log("result vendor_master count :: "+ result);
				if(result!=0)
				{
					if(vendorid)
			  		{
...
if(id)
{
	console.log("PO View page ");
		...
		var fields_po_ns_details= {key:"PO_NS_ID",operator:"=",value:po_ns_id};
		...
		*/
		...
		var fields_po_line_contents= {key:"PO_NS_ID",operator:"=",value:po_ns_id};
...
if(id)
{
	...
	{
		var fields_plstatus_details= {key:"STATUS",operator:"=",value:pl_status};
	...
	if(pl_line_contents!='' || pl_line_contents!=null)
	{
...
if(id)
{
	...
	var fields_plid_details= {key:"PO_NS_ID",operator:"=",value:po_ns_id};
		dbs.query('purchase_order',fields_plid_details);
		dbs.readData(function (result){
		console.log("result purchase order details :: "+ JSON.stringify(result));
...
var fields_poid_detail= {key:"PO_NS_ID",operator:"=",value:po_ns_id};
		dbs.query('po_lines',fields_poid_detail);
		dbs.readData(function (result){
		console.log("result purchase order lines details :: "+ JSON.stringify(result));
		...
		po_line_result=result;
		var items_data = new Array();
		
		for (var i=0; i<po_line_result.length; i++) 
		{
			var item = new Object();
			var qty_dispt=0;
			
			item.item_id_val= po_line_result[i].ITEM_ID;
			console.log("item.item_id_val  "+item.item_id_val);
			
			
			
		var fields_plid_detail= {key:"PO_NS_ID",operator:"=",value:po_ns_id};
		dbs.query('packing_list_lines',fields_plid_detail);
		dbs.readData(function (result){
		console.log("result purchase order lines details :: "+ JSON.stringify(result));
			...
			if(result!='' || result!=null)
			{
				...
				{
					qty_dispt += result[j].QTY_DISPATCHED;
				}
				console.log(" item qty_dispt"+qty_dispt);
			}
			
			item.item_name_val = po_line_result[i].ITEM_NAME; 
			item.po_ns_id_val = po_line_result[i].PO_NS_ID;
			item.description_val = po_line_result[i].DESCRIPTION;
			item.quantity_val = po_line_result[i].QTY;
...
if(id)
{
	...
	var fields_to_push=new Array();
	var values_to_push=new Array();
	
	fields_to_push.push('SHIP_TO');
	fields_to_push.push('SHIP_DATE');
	fields_to_push.push('DELIVERY_METHOD');
	fields_to_push.push('SHIPMENT_ORIGIN');
	fields_to_push.push('SHIPMENT_POINT');
	fields_to_push.push('STATUS');
	fields_to_push.push('packing_list_num');
	fields_to_push.push('po_num');
	fields_to_push.push('DATE_CREATED');
	fields_to_push.push('MEMO');
	fields_to_push.push('po_ns_id');
	
	console.log('fields_to_push  ::'+fields_to_push);
	
	values_to_push.push("'"+req.body.shipto+"'");
	values_to_push.push("'"+req.body.shipdate+"'");
	values_to_push.push("'"+req.body.deliverymethod+"'");
	values_to_push.push("'"+req.body.shipmentorigin+"'");
	values_to_push.push("'"+req.body.shipmentpoint+"'");
	values_to_push.push('Ready to Ship');
	values_to_push.push("'PL"+req.body.po_num+"'");
	values_to_push.push("'"+req.body.po_num+"'");
	values_to_push.push("'"+req.body.date_created+"'");
	values_to_push.push("'"+req.body.memo+"'");
	values_to_push.push("'"+req.body.po_ns_idval+"'");

	console.log('values_to_push  ::'+values_to_push);
	
		dbs.query('packing_list',fields_to_push,values_to_push);
		dbs.write(function (result){
		console.log("result packing list  details with status all pl :: "+ JSON.stringify(result));
...
if(id)
{
	...
	var fields_plnum_details= {key:"packing_list_num",operator:"=",value:packinglist_num};
		dbs.query('packing_list',fields_plnum_details);
		dbs.readData(function (result){
		console.log("result packing list details :: "+ JSON.stringify(result));
	var pl_contents=result;
	
	if(pl_contents!=null || pl_contents!='')
	{
	
	console.log("pl_contents length : "+pl_contents.length);
	
	for (var i=0; i<pl_contents.length; i++) 
	{
		ship_to_val = pl_contents[i].SHIP_TO;
		ship_date_val = pl_contents[i].SHIP_DATE;
		delivery_method_val = pl_contents[i].DELIVERY_METHOD;
		ship_origin_val = pl_contents[i].SHIPMENT_ORIGIN;
		shipment_point_val = pl_contents[i].SHIPMENT_POINT;
		pl_num_val = pl_contents[i].packing_list_num;
		memo_val = pl_contents[i].MEMO;
		status_val = pl_contents[i].STATUS;
	...
	var fields_plnum_details= {key:"packing_list_num",operator:"=",value:packinglist_num};
		...
		dbs.readData(function (result){
		console.log("result packing list details :: "+ JSON.stringify(result));
	var pl_line_contents=result;
	if(pl_line_contents !='' || pl_line_contents!=null)
	{
		
		console.log("pl_line_contents length : "+pl_line_contents.length);
		...
		for (var i=0; i<pl_line_contents.length; i++) 
		{
			...
			item.item_name= pl_line_contents[i].ITEM_NAME;
			item.item_desc = pl_line_contents[i].DESCRIPTION;
			item.quantity = pl_line_contents[i].QTY_ORDERED;
			item.dispatched_qty = pl_line_contents[i].QTY_DISPATCHED;
			item.net_wt = pl_line_contents[i].NET_WT;
			item.gross_wt = pl_line_contents[i].GROSS_WT;
			item.po_ref_num = pl_line_contents[i].REF_PO_NUM;
...
if(id)
{
	...
	if(recordType=='Purchase Order')
	{
		...
		var fields_searchContents= {key:"po_number",operator:"=",value:searchKeyValue};
	...
	*/	
	...
	//	var searchContents = db.exec("select * from packing_list where packing_list_num='"+searchKeyValue+"'");
		...
		var fields_searchContents= {key:"packing_list_num",operator:"=",value:searchKeyValue};
